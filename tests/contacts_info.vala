...
public async void playground() {
	...
	try {
		...
		foreach(var acco in acm.valid_accounts) {
			...
			ConnectionInterfaceContacts cinfo =  null;
			GLib.HashTable<uint, GLib.HashTable<string, GLib.Variant>> hash_info;
			try {
				string[] interfaces = {};
				...
				cinfo = Bus.get_proxy_sync (BusType.SESSION, "org.freedesktop.Telepathy.ConnectionManager.gabble", acc.connection.to_string());
				hash_info = cinfo.get_contact_attributes(chg.members, interfaces, true);
			} catch (Error erb) {
				stderr.printf("ERR: Could not get ConnectionInterfaceContacts with path %s: %s\n", acc.connection.to_string(), erb.message);
				continue;
			}
			...
			foreach(var member in chg.members) {
					...
					var hash_member = hash_info.lookup(member);
					if(hash_member==null) continue;
					hash_member.@foreach( (key, val) => { 
